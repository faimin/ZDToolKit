...
@implementation ZDGuardUIKitOnMainThread
...
__attribute__((constructor)) static void ZD_UIKitMainThreadGuard(void) {
    @autoreleasepool {
        for (NSString *selStr in @[PROPERTY(setNeedsLayout), PROPERTY(setNeedsDisplay), PROPERTY(setNeedsDisplayInRect:)]) {
            ...
            SEL newSelector = NSSelectorFromString([NSString stringWithFormat:@"ZD_%@", selStr]);
            if ([selStr hasSuffix:@":"]) {
                ZD_ReplaceMethodWithBlock(UIView.class, selector, newSelector, ^(__unsafe_unretained UIView *_self, CGRect r) {
                     ...
                     */
                    if (_self.window) ZD_AssertIfNotMainThread();
                    ((void (*)(id, SEL, CGRect))objc_msgSend)(_self, newSelector, r);
                });
            } else {
                ZD_ReplaceMethodWithBlock(UIView.class, selector, newSelector, ^(__unsafe_unretained UIView *_self) {
                    if (_self.window) {
...
#pragma clang diagnostic pop
                            ...
                            */
                            ...
                            if (!strstr(dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL), "UIKit")) {
                                ZD_AssertIfNotMainThread();
                            }
                        }
                    }

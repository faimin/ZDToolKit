...
- (NSArray *)zd_arguments {
    NSUInteger numberOfArguments = self.methodSignature.numberOfArguments;
    NSMutableArray *argumentsArray = [NSMutableArray arrayWithCapacity:numberOfArguments - 2];
    for (NSUInteger index = 2; index < numberOfArguments; index++) {
        [argumentsArray addObject:[self zd_argumentAtIndex:index] ?: NSNull.null];
    }
    
    return argumentsArray;
}

- (void)setZd_arguments:(NSArray *)arguments {
    ...
    for (id arg in arguments) {
        [self zd_setArgument:(arg == NSNull.null ? nil : arg) atIndex:index];
        index++;
    }
}

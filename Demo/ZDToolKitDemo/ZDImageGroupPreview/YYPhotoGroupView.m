...
#if (__has_include(<YYWebImage/YYWebImage.h>) || __has_include(<YYKit/YYKit.h>))
...
- (void)setItem:(YYPhotoGroupItem *)item {
    ...
    [_imageView.layer __removePreviousFadeAnimation];
    ...
    [_imageView yy_setImageWithURL:item.largeImageURL placeholder:item.thumbImage options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        ...
        if (stage == YYWebImageStageFinished) {
            ...
            if (image) {
                ...
                [self.imageView.layer __addFadeAnimationWithDuration:0.1 curve:UIViewAnimationCurveLinear];
            }
        }
    }];
    [self resizeSubviewSize];
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    _snapshotImage = [_toContainerView zd_snapshotImageAfterScreenUpdates:NO];
    BOOL fromViewHidden = fromView.hidden;
    fromView.hidden = YES;
    _snapshorImageHideFromView = [_toContainerView zd_snapshotImage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    if (_fromItemIndex != currentPage) {
        ...
        [_background.layer __addFadeAnimationWithDuration:0.25 curve:UIViewAnimationCurveEaseOut];
    } else {
        _background.image = _snapshorImageHideFromView;
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    intPage = intPage < 0 ? 0 : (intPage >= _groupItems.count ? _groupItems.count - 1 : intPage);
...
- (void)showHUD:(NSString *)msg {
    ...
    label.size = ({
        ...
        CGSizeMake(ceil(size.width * scale) / scale,
...
- (void)longPress {
    ...
    UIViewController *toVC = self.toContainerView.zd_viewController;
    if (!toVC) toVC = self.zd_viewController;
...
@implementation CALayer (YY)

static NSString *_YYUIViewContentModeToCAGravity(UIViewContentMode contentMode) {
    switch (contentMode) {
...
- (void)__setContentMode:(UIViewContentMode)contentMode {
    self.contentsGravity = _YYUIViewContentModeToCAGravity(contentMode);
}

- (void)__addFadeAnimationWithDuration:(NSTimeInterval)duration curve:(UIViewAnimationCurve)curve {
...
- (void)__removePreviousFadeAnimation {
